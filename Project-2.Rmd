...
lm2Fit <- lm(shares ~ . + n_tokens_title:average_token_length +
...
boostedFit <- train(shares ~ ., data = world_news_train, 
                    method = "gbm",
                    trControl = control,
...
lm1 <- round(postResample(prediction, world_news_test$shares), 3)

prediction <- predict(lm2Fit, newdata = world_news_test)
lm2 <- round(postResample(prediction, world_news_test$shares), 3)

prediction <- predict(boostedFit, newdata = world_news_test)
boost <- round(postResample(prediction, world_news_test$shares), 3)

compareFits <- data.frame(lm1, lm2, boost)
